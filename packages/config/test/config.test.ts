...
import { ConfigValidationError, loadEdgeServiceConfig, loadServiceConfig } from '../index.ts';
...
function createEdgeEnv(overrides: Record<string, string> = {}): Record<string, string> {
  return {
...
test('loads edge service configuration without DATABASE_URL', () => {
  const config = loadEdgeServiceConfig(createEdgeEnv());

  assert.equal(config.serviceName, 'api-gateway');
  assert.equal(config.port, 3010);
  assert.deepEqual(config.kafkaBootstrapServers, ['localhost:9092']);
  assert.equal(config.otelExporterOtlpEndpoint, 'http://localhost:4318');
  assert.equal(config.logLevel, 'info');
});

test('edge loader throws when required edge variables are missing', () => {
  assert.throws(
    () =>
    ...
    (error) => {
      assert.ok(error instanceof ConfigValidationError);
      assert.match(error.message, /SERVICE_NAME is required/);
      assert.doesNotMatch(error.message, /DATABASE_URL is required/);
      return true;
    }
  );
});
