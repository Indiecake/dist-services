...
| `inventory-service` | `inventory_schema` | Reservation state, stock movements, inbox/outbox, reservation items, and dead letters |
...
- `inventory-service` local catalog rows (`sku-1`, `sku-2`) are applied with `pnpm --filter @services-sandbox/inventory-service db:seed`, not via SQL migrations.
